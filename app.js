...
import { readDefinition, validateDefinition, connectionIssues, requiredTables, compileSas, parseCodes } from './cohort.js?v=20580f35e743';
import { openCodePicker } from './code-picker.js?v=20580f35e743';
import { treeFor, groupsIn, logicText, usesOr, removeCriterion } from './logic.js?v=20580f35e743';
import { renderTree, bindTree } from './cohort-tree.js?v=20580f35e743';
import { selectionProtocol } from './protocol.js?v=20580f35e743';
import { parseCsv, previewRows, quickCounts, missingness, compileQuickCount } from './results.js?v=20580f35e743';
import { parseRunFolder, parseOutputParent } from './paths.js?v=20580f35e743';
import { PROFILE_KEY, DEFAULT_LINK_SCRIPT, createProfile, readProfileStore, profileFromSettings } from './profiles.js?v=20580f35e743';
import { SAVED_COHORTS_KEY, LEGACY_SAVED_COHORTS_KEY, RETIRED_COHORTS_KEY, partitionSavedCohorts, readSavedCohorts, upsertSavedCohort, compareDefinitions, comparisonReport, definitionSha256 } from './saved-cohorts.js?v=20580f35e743';
import { elapsedLabel, jobProgressText } from './job-progress.js?v=20580f35e743';
...
function render() {
  ...
  if(view==='graph'||view==='saved'&&selectedSavedCohortId)bindTree(app.querySelector('.tree-workspace'),definition,catalog,{
    ...
    browseCodes:(kind,index)=>{const r=kind==='cov'?definition.covariates[index]:eventRule(index);openCodePicker({domain:r.domain,codes:r.codes,label:catalog.domains[r.domain].label,onApply:codes=>{r.codes=codes;changed();render();}}).catch(error=>toast(error.message));},
...
document.addEventListener('click', e=>{
  ...
  if(action==='add-covariate'){const n=definition.covariates.length+1;definition.covariates.push({key:`feature_${n}`,label:`Feature ${n}`,domain:'DX',sources:['DIA'],codes:'',encTypes:Object.keys(cdm.ENC_TYPES),from:-365,to:-1,minDays:1,...(definition.comparison?{arm:e.target.closest('[data-action]').dataset.arm||'BOTH'}:{})});changed();render();document.querySelector(`#cov-${n-1}-key`)?.focus();}
...
try {
  ...
  const responses=await Promise.all([fetch('./cdm_engine.sas?v=20580f35e743')]);
