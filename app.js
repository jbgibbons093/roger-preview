import * as cdm from './cdm.js?v=b247f1c0d48e';
import { readDefinition, validateDefinition, connectionIssues, requiredTables, compileSas, parseCodes } from './cohort.js?v=b247f1c0d48e';
import { openCodePicker } from './code-picker.js?v=b247f1c0d48e';
import { treeFor, groupsIn, logicText, usesOr, removeCriterion } from './logic.js?v=b247f1c0d48e';
import { renderTree, bindTree } from './cohort-tree.js?v=b247f1c0d48e';
import { selectionProtocol } from './protocol.js?v=b247f1c0d48e';
import { parseCsv, previewRows, quickCounts, missingness, compileQuickCount } from './results.js?v=b247f1c0d48e';
import { parseRunFolder, parseOutputParent } from './paths.js?v=b247f1c0d48e';
import { PROFILE_KEY, DEFAULT_LINK_SCRIPT, createProfile, readProfileStore, profileFromSettings } from './profiles.js?v=b247f1c0d48e';
import { SAVED_COHORTS_KEY, LEGACY_SAVED_COHORTS_KEY, RETIRED_COHORTS_KEY, partitionSavedCohorts, readSavedCohorts, upsertSavedCohort, compareDefinitions, comparisonReport, definitionSha256 } from './saved-cohorts.js?v=b247f1c0d48e';
import { elapsedLabel, jobProgressText } from './job-progress.js?v=b247f1c0d48e';
...
function ruleEditor(r, index) {
  const id = index === -1 ? 'index' : index === -2 ? 'control-index' : `rule-${index}`;
  ...
  return `<div class="fields">
    ...
    ${index === -1 ? `<div><label for="index-anchor">Index date within each group</label><select id="index-anchor" data-field="indexOrder">${option('FIRST','First matching event in date range',definition.indexOrder)}${option('LAST','Last matching event in date range',definition.indexOrder)}</select></div>` : index === -2 ? '<p class="hint">Uses the first/last setting above, independently within the control group.</p>' : `<div><label for="${id}-mode">Eligibility rule</label><select id="${id}-mode" data-rule="${index}" data-key="mode">${option('INCLUDE','Include people with',r.mode)}${option('EXCLUDE','Exclude people with',r.mode)}</select></div>`}
    ...
    ${isIndex ? '' : `<div class="full fields three"><div><label for="${id}-from">From day</label><input id="${id}-from" type="number" min="-3650" max="3650" value="${r.from}" data-rule="${index}" data-key="from"></div><div><label for="${id}-to">Through day</label><input id="${id}-to" type="number" min="-3650" max="3650" value="${r.to}" data-rule="${index}" data-key="to"></div><div><label for="${id}-days">Minimum distinct days</label><input id="${id}-days" type="number" min="1" max="7301" value="${r.minDays}" data-rule="${index}" data-key="minDays"></div></div><p class="hint full">Day 0 is the index date. Negative days precede index. Both endpoints are included.</p>`}
  </div>`;
}
function comparisonPanel(){
  const c=definition.comparison;
  return `<div class="comparison-builder"><label class="check-row"><input type="checkbox" data-comparison-toggle ${c?'checked':''}>Define a separate control index</label>${c?`<p class="hint">Each person can have one treatment candidate date and one control candidate date. The selected study index determines group assignment; both candidate dates are retained for audit.</p><div class="fields three"><div><label for="control-start">Control index from</label><input id="control-start" type="date" data-comparison-field="start" value="${esc(c.start)}"></div><div><label for="control-end">Through</label><input id="control-end" type="date" data-comparison-field="end" value="${esc(c.end)}"></div><div><label for="comparison-overlap">If both qualify</label><select id="comparison-overlap" data-comparison-field="overlap">${option('EARLIEST','Earlier candidate date wins; treatment wins a tie',c.overlap)}${option('EXCLUDE','Exclude people in both groups',c.overlap)}</select></div></div><h3>Control index event</h3>${ruleEditor(c.controlIndex,-2)}`:'<p class="hint">Enable this to build treatment and control groups with separate code sets and index date ranges.</p>'}</div>`;
}
function eventRule(index){return index===-1?definition.index:index===-2?definition.comparison?.controlIndex:definition.rules[index];}
function covariatePanel(){
  if(definition.comparison){
    const scopes=[['BOTH','Shared covariates'],['TREATMENT','Treatment covariates'],['CONTROL','Control covariates']];
    const card=(r,i)=>`<div class="rule"><div class="rule-head"><span class="rule-label">${esc(r.label||r.key||`Covariate ${i+1}`)}</span><button type="button" data-cov-remove="${i}" aria-label="Remove covariate ${i+1}">×</button></div><div class="rule-body"><div class="fields"><div><label for="cov-${i}-key">SAS variable key</label><input id="cov-${i}-key" data-cov="${i}" data-key="key" value="${esc(r.key)}" maxlength="20"></div><div><label for="cov-${i}-label">Display label</label><input id="cov-${i}-label" data-cov="${i}" data-key="label" value="${esc(r.label)}" maxlength="80"></div><div><label for="cov-${i}-arm">Applies to</label><select id="cov-${i}-arm" data-cov="${i}" data-key="arm">${scopes.map(([v,label])=>option(v,label,r.arm||'BOTH')).join('')}</select></div><div><label for="cov-${i}-domain">Code system</label><select id="cov-${i}-domain" data-cov="${i}" data-key="domain">${Object.keys(cdm.DOMAINS).map(k=>option(k,cdm.catalog.domains[k].label,r.domain)).join('')}</select></div><div class="full"><label for="cov-${i}-codes">Codes</label><textarea id="cov-${i}-codes" data-cov="${i}" data-key="codes" rows="2">${esc(r.codes)}</textarea><button class="button small" type="button" data-cov-browse="${i}">Browse codes</button></div><div class="full"><span class="field-label">Encounter types</span><div class="source-checks">${r.domain==='NDC'?'<span class="hint">Not applicable to dispensing.</span>':Object.entries(cdm.ENC_TYPES).map(([value,label])=>`<label class="check-row"><input type="checkbox" data-cov-enc="${i}" value="${value}" ${r.encTypes.includes(value)?'checked':''}>${value} · ${label}</label>`).join('')}</div></div><div><label for="cov-${i}-from">From index day</label><input id="cov-${i}-from" type="number" data-cov="${i}" data-key="from" value="${r.from}"></div><div><label for="cov-${i}-to">Through index day</label><input id="cov-${i}-to" type="number" data-cov="${i}" data-key="to" value="${r.to}"></div><div><label for="cov-${i}-days">Minimum distinct days</label><input id="cov-${i}-days" type="number" min="1" data-cov="${i}" data-key="minDays" value="${r.minDays}"></div></div></div></div>`;
    return panel('05','Treatment and control covariates','Build shared features or features that apply to one group.',`<p class="hint">All windows are relative to the selected index date. A feature assigned to only one group is missing (not applicable) in the other group; it is never coded as absence of the condition.</p>${scopes.map(([arm,title])=>`<section class="covariate-group"><h3>${title}</h3>${definition.covariates.map((r,i)=>(r.arm||'BOTH')===arm?card(r,i):'').join('')||'<p class="hint">No features in this group.</p>'}<button class="add-rule" type="button" data-action="add-covariate" data-arm="${arm}" ${definition.covariates.length>=20?'disabled':''}>+ Add ${arm==='BOTH'?'shared':arm.toLowerCase()} covariate</button></section>`).join('')}`);
  }
...
function predictorPanel(){
  const p=definition.predictors||{age:true,sex:true};
  return panel('06','Predictor selection','Mark variables intended for baseline adjustment.',`<p class="hint">Only pre-index covariates can be marked as predictors. All calculated variables remain in COHORT; PREDICTOR_SPECS records your selection for later analyses.</p><div class="source-checks"><label class="check-row"><input type="checkbox" data-predictor-built-in="age" ${p.age?'checked':''}>Age at index</label><label class="check-row"><input type="checkbox" data-predictor-built-in="sex" ${p.sex?'checked':''}>Recorded sex</label>${definition.covariates.map((r,i)=>`<label class="check-row"><input type="checkbox" data-predictor-cov="${i}" ${r.predictor===true?'checked':''}>${esc(r.label||r.key)} · cov_${esc(r.key)}</label>`).join('')||'<p class="hint">Add a baseline covariate above to select it as a predictor.</p>'}</div>`);
}
function outcomePanel(){
  const outcomes=definition.outcomes||[];
  const complete=r=>cdm.outcomeFollowupComplete(definition,r);
  return panel('07','Post-index outcomes','Find the first qualifying event after each person’s selected index date.',`<p class="hint">Day 1 is the default start; index-day events are excluded. COHORT receives out_&lt;key&gt; (1 event, 0 confirmed no event, missing when follow-up is not assured), out_&lt;key&gt;_date, out_&lt;key&gt;_hit (recorded claim), and out_&lt;key&gt;_obs (complete follow-up, accounting for enrollment and recorded death).</p>${outcomes.map((r,i)=>`<div class="rule"><div class="rule-head"><span class="rule-label">${esc(r.label||r.key||`Outcome ${i+1}`)}</span><button type="button" data-outcome-remove="${i}" aria-label="Remove outcome ${i+1}">×</button></div><div class="rule-body"><div class="fields"><div><label for="outcome-${i}-key">SAS variable key</label><input id="outcome-${i}-key" data-outcome="${i}" data-key="key" value="${esc(r.key)}" maxlength="20"></div><div><label for="outcome-${i}-label">Display label</label><input id="outcome-${i}-label" data-outcome="${i}" data-key="label" value="${esc(r.label)}" maxlength="80"></div><div><label for="outcome-${i}-domain">Code system</label><select id="outcome-${i}-domain" data-outcome="${i}" data-key="domain">${Object.keys(cdm.DOMAINS).map(k=>option(k,cdm.catalog.domains[k].label,r.domain)).join('')}</select></div><div><label>Source table</label><div class="source-pill">${esc(cdm.catalog.tables[r.sources[0]].label)}</div></div><div class="full"><label for="outcome-${i}-codes">Codes</label><textarea id="outcome-${i}-codes" data-outcome="${i}" data-key="codes" rows="2">${esc(r.codes)}</textarea><button class="button small" type="button" data-outcome-browse="${i}">Browse codes</button></div><div class="full"><span class="field-label">Encounter types</span><div class="source-checks">${r.domain==='NDC'?'<span class="hint">Not applicable to dispensing.</span>':Object.entries(cdm.ENC_TYPES).map(([value,label])=>`<label class="check-row"><input type="checkbox" data-outcome-enc="${i}" value="${value}" ${r.encTypes.includes(value)?'checked':''}>${value} · ${label}</label>`).join('')}</div></div><div><label for="outcome-${i}-from">From index day</label><input id="outcome-${i}-from" type="number" min="1" max="3650" data-outcome="${i}" data-key="from" value="${r.from}"></div><div><label for="outcome-${i}-to">Through index day</label><input id="outcome-${i}-to" type="number" min="1" max="3650" data-outcome="${i}" data-key="to" value="${r.to}"></div></div><p class="hint">${complete(r)?'Current enrollment settings support a no-event conclusion if no death is recorded before this horizon.':'No-event results will be missing unless uninterrupted medical coverage (and drug coverage for NDC outcomes) is required through this horizon.'}</p></div></div>`).join('')||'<div class="empty">No post-index outcomes selected yet.</div>'}<button class="add-rule" type="button" data-action="add-outcome" ${outcomes.length>=20?'disabled':''}>+ Add outcome</button>`);
}
function summary() {
  ...
  return `<aside class="summary panel"><div class="summary-head"><p class="eyebrow">YOUR COHORT</p><div class="summary-title">${esc(definition.name || 'Untitled cohort')}</div><p class="hint">${esc(populationLabel())}</p></div><div class="summary-body">
    <div class="rule-count">${definition.rules.length + 1 + (definition.comparison?1:0)}<small>event criteria in this definition</small></div><hr>
    <dl><dt>Index code list</dt><dd>${count === null ? 'Check codes' : `${count} ${count === 1 ? 'code' : 'codes'}`}</dd><dt>${ageLabel()}</dt><dd>${definition.ageMin}–${definition.ageMax}</dd><dt>Enrollment</dt><dd>${definition.enrollment ? 'Required' : 'Optional'}</dd><dt>Covariates</dt><dd>${definition.covariates.length}</dd><dt>Outcomes</dt><dd>${definition.outcomes?.length||0}</dd><dt>Claim extracts</dt><dd>${definition.outputs.length} tables</dd></dl>
...
function populationFields(){
  ...
  return `<div class="fields"><div class="full">${field('Cohort name','name',definition.name)}</div>
    ...
    ${field(definition.comparison?'Treatment index dates from':'Index dates from','start',definition.start,'date',`min="${min}" max="${max}"`)}${field('Through','end',definition.end,'date',`min="${min}" max="${max}"`)}
...
function builder() {
  return `<div class="builder-grid"><div class="stack">
    ...
    ${panel('02', definition.comparison?'Treatment and control index':'Index event', 'The selected matching event establishes each person’s index date.', ruleEditor(definition.index,-1) + '<p class="hint" style="margin-top:15px">Demographics and subsequent rules apply after the index event is selected. Other candidate events do not replace an ineligible selected event.</p>' + comparisonPanel())}
...
function review() {
  ...
  return `<div class="review-grid"><div class="stack">${panel('01','Review the definition','Check the population, temporal logic, and requested records.', `<p class="review-summary-text">${esc(definition.name)} selects people in <strong>${esc(populationLabel())}</strong> with the ${definition.indexOrder==='LAST'?'last':'first'} matching event between <strong>${fmtDate(definition.start)}</strong> and <strong>${fmtDate(definition.end)}</strong>. ${ageLabel()} must be <strong>${definition.ageMin}–${definition.ageMax}</strong>. ${definition.enrollment ? `Enrollment must cover ${definition.baseline} days before and ${definition.followup} days after index, with internal gaps of at most ${definition.gap} days.` : 'Enrollment is not an eligibility requirement.'} ${definition.sex==='ALL'?'All recorded sex values are accepted.':`Recorded sex must equal ${esc(definition.sex)}.`} ${definition.enrollment?'Covered intervals require MedCov=Y.':''} ${definition.rx?'Covered intervals also require DrugCov=Y.':''}</p>${definition.comparison?`<p class="review-summary-text">Control index: ${esc(catalog.domains[definition.comparison.controlIndex.domain].label)} codes ${esc(definition.comparison.controlIndex.codes)} from ${fmtDate(definition.comparison.start)} through ${fmtDate(definition.comparison.end)}. ${definition.comparison.overlap==='EXCLUDE'?'People with both candidate dates are excluded.':'The earlier candidate date determines the group; treatment wins a same-day tie.'} Each person has one selected index date. Shared and arm-specific covariates are calculated relative to that date.</p>`:''}<p class="review-summary-text">${definition.outputs.length ? `Extract ${definition.outputs.join(', ')}. Clinical records span ${definition.extractBefore} days before through ${definition.extractAfter} days after index. Demographic and Death include all selected-person records. Annual files remain separate.` : 'Export the cohort and audit tables only.'}</p><div class="logic-summary"><strong>${esc(logicText(treeFor(definition)))}</strong><button class="button small" data-action="graph">Edit tree</button></div>${rules.map((r,i)=>`<div class="review-rule"><span class="step-number">${i+1}</span><div><h3>${r.mode==='INDEX'?(definition.comparison?'Treatment index event':'Index event'):r.mode==='INCLUDE'?'Required event':'Exclusion event'} · ${catalog.domains[r.domain].label}</h3><p>${r.sources.map(t=>catalog.tables[t].label).join(', ')}${r.domain!=='NDC'?` · EncType ${r.encTypes.join(', ')}`:''}</p>${i?`<p>At least ${r.minDays} distinct day${r.minDays===1?'':'s'} from day ${r.from} through day ${r.to}, inclusive.</p>`:`<p>${definition.indexOrder==='LAST'?'Last':'First'} observed match within the index date range.</p>`}<div>${esc(r.codes).split(/[\s,;]+/).filter(Boolean).slice(0,30).map(c=>`<span class="code-chip">${c}</span>`).join('')}${r.codes.split(/[\s,;]+/).filter(Boolean).length>30?'<span class="hint">More codes are included in the program.</span>':''}</div></div></div>`).join('')}`)}
...
function diagnostics(){
  ...
  const sasTables=['diagnostics','attrition','counts','missingness','extract_counts','covariate_specs','predictor_specs','outcome_specs'].filter(name=>resultTables.has(name));
...
function render() {
  ...
  if(view==='graph'||view==='saved'&&selectedSavedCohortId)bindTree(app.querySelector('.tree-workspace'),definition,catalog,{
    ...
    browseCodes:(kind,index)=>{const r=kind==='cov'?definition.covariates[index]:kind==='outcome'?definition.outcomes[index]:eventRule(index);openCodePicker({domain:r.domain,codes:r.codes,label:catalog.domains[r.domain].label,onApply:codes=>{r.codes=codes;changed();render();}}).catch(error=>toast(error.message));},
...
document.addEventListener('click', e=>{
  ...
  if(browse){const index=Number(browse.dataset.browse), r=eventRule(index);openCodePicker({domain:r.domain,codes:r.codes,label:catalog.domains[r.domain].label,onApply:codes=>{r.codes=codes;document.querySelector(`#${index===-1?'index':index===-2?'control-index':`rule-${index}`}-codes`).value=codes;changed();updateSummary();}}).catch(error=>toast(error.message));return;}
  ...
  const outcomeBrowse=e.target.closest('[data-outcome-browse]');
  if(outcomeBrowse){const r=definition.outcomes[Number(outcomeBrowse.dataset.outcomeBrowse)];openCodePicker({domain:r.domain,codes:r.codes,label:cdm.catalog.domains[r.domain].label,onApply:codes=>{r.codes=codes;changed();render();}}).catch(error=>toast(error.message));return;}
  ...
  if(action==='add-covariate'){const n=definition.covariates.length+1;definition.covariates.push({key:`feature_${n}`,label:`Feature ${n}`,domain:'DX',sources:['DIA'],codes:'',encTypes:Object.keys(cdm.ENC_TYPES),from:-365,to:-1,minDays:1,predictor:true,...(definition.comparison?{arm:e.target.closest('[data-action]').dataset.arm||'BOTH'}:{})});changed();render();document.querySelector(`#cov-${n-1}-key`)?.focus();}
  ...
  if(action==='add-outcome'){definition.outcomes||=[];const n=definition.outcomes.length+1;definition.outcomes.push({key:`outcome_${n}`,label:`Outcome ${n}`,domain:'DX',sources:['DIA'],codes:'',encTypes:Object.keys(cdm.ENC_TYPES),from:1,to:30});changed();render();document.querySelector(`#outcome-${n-1}-key`)?.focus();}
  const outcomeRemove=e.target.closest('[data-outcome-remove]');if(outcomeRemove){definition.outcomes.splice(Number(outcomeRemove.dataset.outcomeRemove),1);changed();render();}
...
app.addEventListener('input',e=>{
  ...
  if(el.dataset.cov!==undefined&&el.dataset.key){const r=definition.covariates[Number(el.dataset.cov)];r[el.dataset.key]=el.type==='number'?(el.value===''?NaN:Number(el.value)):el.value;changed();if(el.dataset.key==='arm')render();else updateSummary();return;}
  if(el.dataset.outcome!==undefined&&el.dataset.key){const r=definition.outcomes[Number(el.dataset.outcome)];r[el.dataset.key]=el.type==='number'?(el.value===''?NaN:Number(el.value)):el.value;changed();updateSummary();return;}
  if(el.dataset.comparisonField){definition.comparison[el.dataset.comparisonField]=el.value;changed();updateSummary();return;}
  ...
  if(el.dataset.rule!==undefined){const r=eventRule(Number(el.dataset.rule));r[el.dataset.key]=el.type==='number'?(el.value===''?NaN:Number(el.value)):el.value;changed();updateSummary();}
});
app.addEventListener('change',e=>{
  const el=e.target;
  if(el.dataset.comparisonToggle!==undefined){
    if(el.checked)definition.comparison={start:definition.start,end:definition.end,overlap:'EARLIEST',controlIndex:{...structuredClone(definition.index),codes:''}};
    else{delete definition.comparison;for(const cov of definition.covariates)delete cov.arm;}
  ...
  if(el.dataset.predictorBuiltIn!==undefined){definition.predictors||={age:true,sex:true};definition.predictors[el.dataset.predictorBuiltIn]=el.checked;changed();updateSummary();return;}
  if(el.dataset.predictorCov!==undefined){definition.covariates[Number(el.dataset.predictorCov)].predictor=el.checked;changed();updateSummary();return;}
  if(el.dataset.outcomeEnc!==undefined){const r=definition.outcomes[Number(el.dataset.outcomeEnc)];r.encTypes=el.checked?[...r.encTypes,el.value]:r.encTypes.filter(value=>value!==el.value);changed();updateSummary();return;}
  if(el.dataset.outcome!==undefined&&el.dataset.key==='domain'){const r=definition.outcomes[Number(el.dataset.outcome)];r.sources=cdm.DOMAINS[r.domain].slice();r.codes='';r.encTypes=r.domain==='NDC'?[]:Object.keys(cdm.ENC_TYPES);changed();render();return;}
  ...
  if(el.dataset.encType!==undefined){const r=eventRule(Number(el.dataset.encType));r.encTypes=el.checked?[...r.encTypes,el.value]:r.encTypes.filter(t=>t!==el.value);changed();render();}
  if(el.dataset.ruleSource!==undefined){const r=eventRule(Number(el.dataset.ruleSource));r.sources=el.checked?[...r.sources,el.value]:r.sources.filter(t=>t!==el.value);changed();render();}
  ...
  if(el.dataset.key==='domain'){const r=eventRule(Number(el.dataset.rule));r.sources=DOMAINS[r.domain].slice(0,2);r.codes='';changed();render();}
...
try {
  ...
  const responses=await Promise.all([fetch('./cdm_engine.sas?v=b247f1c0d48e')]);
