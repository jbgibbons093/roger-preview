...
function addCriterion(target){
  ...
  definition.rules.push({domain:'DX',sources:['O','S'],codes:'',mode:'INCLUDE',from:-90,to:-1,minDays:1});changed();return index;
}
function openEventEditor(index){
  ...
  dialog.addEventListener('change',e=>{const el=e.target;if(el.dataset.ruleSource!==undefined)r.sources=el.checked?[...r.sources,el.value]:r.sources.filter(t=>t!==el.value);if(el.dataset.key==='domain'){r.domain=el.value;r.sources=DOMAINS[r.domain].slice(0,2);r.codes='';draw();}changed();});
...
app.addEventListener('change',e=>{
  ...
  if(['enrollment','rx','family','edition','indexOrder'].includes(el.dataset.field)){if(!definition.enrollment)definition.rx=false;if(['family','edition'].includes(el.dataset.field))definition.mapping=Object.fromEntries(TABLES.map(t=>[t,'']));render();}
});
