...
const DRAFT_KEY = 'roger.cohort.cdm.v1', LEGACY_DRAFT_KEY = 'roger.cohort.v1';
...
function mappingPanel() {
  ...
  return `<details class="panel"><summary class="details-toggle">SAS library and table mappings <span class="hint">· ${connectionIssues(definition).length} to configure</span></summary><div class="details-body"><p class="hint" style="margin-top:0;margin-bottom:18px">Dataset names vary by delivery. ${activeCdm()?'The defaults follow Jun’s filenames. {start} and {end} use the delivery years. {year} expands to every annual file. The 2013–2023 range is an editable example.':`Enter two-level names for ${catalog.families[definition.family]} Set ${definition.edition}.`} The exported program stops if required mappings are missing.</p><div class="fields">
...
function populationFields(){
  ...
  return `<div class="fields"><div class="full">${field('Cohort name','name',definition.name)}</div>
    ${c?`${number('Delivery start year','yearStart',definition.yearStart,1900,2100)}${number('Delivery end year','yearEnd',definition.yearEnd,1900,2100)}<p class="hint full">2013–2023 is Jun’s example range. Set the years to your actual delivery. Each annual clinical table is required for every selected delivery year.</p>`:`<div><label for="family">Database</label><select id="family" data-field="family">${Object.entries(catalog.families).map(([k,v])=>option(k,v,definition.family)).join('')}</select></div><div><label for="edition">Delivery edition</label><select id="edition" data-field="edition">${option('A','Set A',definition.edition)}${option('B','Set B',definition.edition)}</select></div>`}
    ...
    <p class="hint full">${c?'Age is completed years from the recorded Birth_Date at the selected index date. Missing or future birth dates are excluded. PatID remains text, preserving leading zeros.':'Age follows MarketScan reporting conventions. The 2023 maximum of 100 includes ages 100 and older. Missing ages are excluded.'}</p></div>`;
}
function cdmCodebook(){
  return `<div class="stack">${panel('CDM','Mini-Sentinel Common Data Model','Version 3.0 · Institutional source layout',`<p class="review-summary-text">The supplied Mini-Sentinel dictionary defines the fields. Jun’s note defines the pooled and annual filenames. Delivery years remain editable, and SAS checks every required file and field before selection.</p><p class="hint">Source document · Mini-Sentinel_Common-Data-Model.pdf, supplied with this project. The source PDF stays local.</p>`)}
