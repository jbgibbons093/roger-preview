import { freshDefinition, readDefinition, validateDefinition, connectionIssues, requiredTables, compileSas, parseCodes, TABLES, DOMAINS } from './cohort.js';

const DRAFT_KEY = 'roger.cohort.v1';
...
const number = (label, key, value, min = 0, max = 365) => field(label, key, value, 'number', `min="${min}" max="${max}" step="1"`);
...
function ruleEditor(r, index) {
  ...
  return `<div class="fields">
    ...
    ${index === -1 ? `<div><label for="index-anchor">Index date</label><input id="index-anchor" value="First matching event in date range" disabled></div>` : `<div><label for="${id}-mode">Eligibility rule</label><select id="${id}-mode" data-rule="${index}" data-key="mode">${option('INCLUDE','Include people with',r.mode)}${option('EXCLUDE','Exclude people with',r.mode)}</select></div>`}
    <div class="full"><label for="${id}-codes">${r.domain === 'NDC' ? '11-digit National Drug Codes' : 'Code list'}</label><textarea id="${id}-codes" data-rule="${index}" data-key="codes" rows="2" placeholder="${r.domain === 'DX' ? 'e.g. E11* or individual codes separated by commas' : r.domain === 'NDC' ? 'Enter 11-digit codes, preserving leading zeros' : 'Enter codes separated by commas'}">${esc(r.codes)}</textarea><p class="hint">${r.domain === 'NDC' ? 'Exact matches only. Hyphenated or 10-digit NDCs require conversion before entry.' : `Codes within this list use OR. A trailing * matches a prefix. Dots are ignored.${r.domain === 'DX' ? ' Only records marked ICD-10-CM are searched.' : ''}`}</p></div>
    ...
    ${index === -1 ? '' : `<div class="full fields three"><div><label for="${id}-from">From day</label><input id="${id}-from" type="number" min="-365" max="365" value="${r.from}" data-rule="${index}" data-key="from"></div><div><label for="${id}-to">Through day</label><input id="${id}-to" type="number" min="-365" max="365" value="${r.to}" data-rule="${index}" data-key="to"></div><div><label for="${id}-days">Minimum distinct days</label><input id="${id}-days" type="number" min="1" max="366" value="${r.minDays}" data-rule="${index}" data-key="minDays"></div></div><p class="hint full">Day 0 is the index date. Negative days precede index. Both endpoints are included.</p>`}
  </div>`;
}
function summary() {
  ...
  return `<aside class="summary panel"><div class="summary-head"><p class="eyebrow">YOUR COHORT</p><div class="summary-title">${esc(definition.name || 'Untitled cohort')}</div><p class="hint">MarketScan ${catalog.families[definition.family]} · 2023 · Set ${definition.edition}</p></div><div class="summary-body">
    <div class="rule-count">${definition.rules.length + 1}<small>event criteria in this definition</small></div><hr>
    <dl><dt>Index code list</dt><dd>${count === null ? 'Check codes' : `${count} ${count === 1 ? 'code' : 'codes'}`}</dd><dt>Reported age</dt><dd>${definition.ageMin}–${definition.ageMax}</dd><dt>Enrollment</dt><dd>${definition.enrollment ? 'Required' : 'Optional'}</dd><dt>Claim extracts</dt><dd>${definition.outputs.length} tables</dd></dl>
    ...
    <hr><div class="status-row"><span class="status-dot"></span>SAS server is not connected</div><p class="hint">Cohort size and attrition become available after a SAS run.</p><button class="button primary full-button" data-action="review">Review &amp; export <span aria-hidden="true">→</span></button>
  </div></aside>`;
}
function mappingPanel() {
  const required = requiredTables(definition);
  return `<details class="panel"><summary class="details-toggle">SAS library and table mappings <span class="hint">· ${connectionIssues(definition).length} to configure</span></summary><div class="details-body"><p class="hint" style="margin-top:0;margin-bottom:18px">Dataset names vary by delivery. Enter the actual two-level SAS names for the selected ${catalog.families[definition.family]} Set ${definition.edition} data. The exported program stops if required mappings are missing.</p><div class="fields">
    ...
    ${required.map(t => `<div><label for="map-${t}">${t} · ${catalog.tables[t].label}</label><input id="map-${t}" data-map="${t}" value="${esc(definition.mapping[t])}" placeholder="LIBREF.TABLE" spellcheck="false" autocomplete="off"></div>`).join('')}
...
function builder() {
  return `<div class="builder-grid"><div class="stack">
    ${panel('01', 'Population', 'Choose the source population and index date range.', `<div class="fields"><div class="full">${field('Cohort name','name',definition.name)}</div><div><label for="family">Database</label><select id="family" data-field="family">${Object.entries(catalog.families).map(([k,v])=>option(k,v,definition.family)).join('')}</select></div><div><label for="edition">Delivery edition</label><select id="edition" data-field="edition">${option('A','Set A',definition.edition)}${option('B','Set B',definition.edition)}</select></div>${field('Index dates from','start',definition.start,'date','min="2023-01-01" max="2023-12-31"')}${field('Through','end',definition.end,'date','min="2023-01-01" max="2023-12-31"')}<div class="full fields three">${number('Minimum reported age','ageMin',definition.ageMin,0,100)}${number('Maximum reported age','ageMax',definition.ageMax,0,100)}<div><label for="sex">Recorded sex</label><select id="sex" data-field="sex">${option('ALL','Any / all recorded values',definition.sex)}${option('1','Male',definition.sex)}${option('2','Female',definition.sex)}</select></div></div><p class="hint full">Age follows MarketScan reporting conventions. The 2023 maximum of 100 includes ages 100 and older. Missing ages are excluded.</p></div>`)}
    ${panel('02', 'Index event', 'The first qualifying event establishes each person’s index date.', ruleEditor(definition.index,-1) + '<p class="hint" style="margin-top:15px">Demographics and subsequent rules apply after the first event is selected. A later event does not replace an ineligible first event.</p>')}
    ${panel('03', 'Eligibility criteria', 'People must satisfy every inclusion rule and avoid every exclusion rule.', `${definition.rules.length ? definition.rules.map((r,i)=>`<div class="rule"><div class="rule-head"><span class="rule-label"><span class="logic ${r.mode==='EXCLUDE'?'exclude':''}">${r.mode === 'INCLUDE' ? 'INCLUDE' : 'EXCLUDE'}</span> &nbsp; Criterion ${i+1}</span><button aria-label="Remove criterion ${i+1}" data-remove="${i}">×</button></div><div class="rule-body">${ruleEditor(r,i)}</div></div>`).join('<div class="and-line">AND</div>') : '<div class="empty">Add diagnoses, procedures, or medications to refine the population.<br>Each criterion can have its own window relative to index.</div>'}<button class="add-rule" data-action="add-rule" ${definition.rules.length>=20?'disabled':''}>+ Add eligibility criterion</button>`)}
    ${panel('04', 'Enrollment & observation', 'Set how much observable time each person needs around index.', `<label class="check-row" style="margin-bottom:20px"><input type="checkbox" data-field="enrollment" ${definition.enrollment?'checked':''}>Require continuous enrollment across the observation window</label><div class="fields three">${number('Days before index','baseline',definition.baseline)}${number('Days after index','followup',definition.followup)}${number('Maximum internal gap','gap',definition.gap)}</div><p class="hint">The window includes index day. Coverage must reach both endpoints. Each internal gap may be at most the selected number of days.</p><label class="check-row" style="margin-top:18px"><input type="checkbox" data-field="rx" ${definition.rx?'checked':''} ${!definition.enrollment?'disabled':''}>Require pharmacy capture throughout the covered intervals</label>`)}
    ${panel('05', 'Data cut', 'Keep one cohort row per person and select the related records to extract.', `<div class="fields">${TABLES.map(t=>`<label class="output-item ${definition.outputs.includes(t)?'selected':''}"><input type="checkbox" data-output="${t}" ${definition.outputs.includes(t)?'checked':''}><span class="table-code">${t}</span><span>${catalog.tables[t].label}<small>${t==='T'?'Intervals overlapping the cut window':t==='I'?'Selected by admission date':'Selected by service date'}</small></span></label>`).join('')}</div><div class="fields" style="margin-top:21px">${number('Extract days before index','extractBefore',definition.extractBefore)}${number('Extract days after index','extractAfter',definition.extractAfter)}</div><p class="hint">The cohort, attrition table, and definition are always included. Claim extracts preserve the original fields and add the index date. Enrollment records preserve their original endpoints.</p>`)}
    ${mappingPanel()}
...
function review() {
  ...
  return `<div class="review-grid"><div class="stack">${panel('01','Review the definition','Check the population, temporal logic, and requested records.', `<p class="review-summary-text">${esc(definition.name)} selects people in <strong>${catalog.families[definition.family]} Set ${definition.edition}</strong> with a first matching event between <strong>${fmtDate(definition.start)}</strong> and <strong>${fmtDate(definition.end)}</strong>. Reported age must be <strong>${definition.ageMin}–${definition.ageMax}</strong>. ${definition.enrollment ? `Enrollment must cover ${definition.baseline} days before and ${definition.followup} days after index, with internal gaps of at most ${definition.gap} days.` : 'Enrollment is not an eligibility requirement.'} ${definition.sex==='ALL'?'All recorded sex values are accepted.':definition.sex==='1'?'Recorded sex must be male.':'Recorded sex must be female.'} ${definition.rx?'Covered intervals must have pharmacy capture.':''}</p><p class="review-summary-text">${definition.outputs.length ? `Extract ${definition.outputs.join(', ')} records from ${definition.extractBefore} days before through ${definition.extractAfter} days after index, inclusive.` : 'Export the cohort and audit tables only.'}</p>${rules.map((r,i)=>`<div class="review-rule"><span class="step-number">${i+1}</span><div><h3>${r.mode==='INDEX'?'Index event':r.mode==='INCLUDE'?'Required event':'Exclusion event'} · ${catalog.domains[r.domain].label}</h3><p>${r.sources.map(t=>catalog.tables[t].label).join(', ')}</p>${i?`<p>At least ${r.minDays} distinct day${r.minDays===1?'':'s'} from day ${r.from} through day ${r.to}, inclusive.</p>`:'<p>First observed match within the index date range.</p>'}<div>${esc(r.codes).split(/[\s,;]+/).filter(Boolean).slice(0,30).map(c=>`<span class="code-chip">${c}</span>`).join('')}${r.codes.split(/[\s,;]+/).filter(Boolean).length>30?'<span class="hint">More codes are included in the program.</span>':''}</div></div></div>`).join('')}`)}
    ...
    <section class="panel"><div class="panel-head"><div><h2>Cohort attrition</h2><p>Counts will be produced by SAS after execution.</p></div></div><table><thead><tr><th>Selection step</th><th>People remaining</th></tr></thead><tbody><tr><td>First matching index event</td><td>Awaiting SAS run</td></tr><tr><td>Demographic requirements</td><td>Awaiting SAS run</td></tr>${definition.enrollment?'<tr><td>Enrollment requirements</td><td>Awaiting SAS run</td></tr>':''}${definition.rules.map((r,i)=>`<tr><td>Criterion ${i+1} · ${r.mode==='INCLUDE'?'Inclusion':'Exclusion'}</td><td>Awaiting SAS run</td></tr>`).join('')}</tbody></table></section>
  </div><aside class="summary panel"><div class="summary-head"><p class="eyebrow">EXPORT PACKAGE</p><h2>Ready for your SAS workspace.</h2></div><div class="summary-body"><p class="review-summary-text" style="font-size:14px">A self-contained SAS 9.4 program with the cohort rules, code lists, selection steps, and requested extracts.</p>${mappingIssues.length?`<div class="notice mapping-notice"><strong>${mappingIssues.length} table mappings remain</strong><br>You can download now and fill in the mappings in the program. SAS will stop until they are supplied.</div>`:'<div class="notice info mapping-notice">Table names are configured. Confirm their database and edition before running.</div>'}<button class="button primary full-button" data-action="export-sas" ${errors.length?'disabled':''}>Download SAS program ↓</button><button class="button full-button" data-action="export-json">Download definition</button><button class="button subtle full-button" data-action="builder">Back to definition</button><hr><p class="export-meta">The SAS engine has been reviewed as source code. Execution validation in SAS 9.4 is pending.</p><p class="hint">For a first check, <a href="./synthetic_fixture.sas" download>download the synthetic SAS check</a> and run it in a fresh SAS session before using research data.</p></div></aside></div>`;
}
function codebook() {
...
function render() {
  ...
  app.innerHTML=view==='builder'?builder():view==='review'?review():codebook();
}
function updateExport() {
  ...
  notice.textContent=errors.length?errors.join(' '):count?`${count} table mappings remain. The exported SAS program will stop until they are supplied.`:'Table names are configured. Confirm their database and edition before running.';
}
...
function filename(extension) { return `${definition.name.toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,'').slice(0,60)||'cohort'}_2023.${extension}`; }
document.addEventListener('click', e=>{
  const viewButton=e.target.closest('[data-view]'); if(viewButton){setView(viewButton.dataset.view);return;}
  const action=e.target.closest('[data-action]')?.dataset.action;
  if(action==='review'||action==='builder'){setView(action);return;}
  if(action==='add-rule'){definition.rules.push({domain:'DX',sources:['O','S'],codes:'',mode:'INCLUDE',from:-90,to:-1,minDays:1});changed();render();document.querySelector(`#rule-${definition.rules.length-1}-codes`)?.focus();}
  const remove=e.target.closest('[data-remove]'); if(remove){definition.rules.splice(Number(remove.dataset.remove),1);changed();render();}
...
app.addEventListener('change',e=>{
  ...
  if(['enrollment','rx','family','edition'].includes(el.dataset.field)){if(!definition.enrollment)definition.rx=false;if(['family','edition'].includes(el.dataset.field))definition.mapping=Object.fromEntries(TABLES.map(t=>[t,'']));render();}
});
document.querySelector('#review-button').onclick=()=>setView('review');
document.querySelector('#save-button').onclick=()=>{
  try{localStorage.setItem(DRAFT_KEY,JSON.stringify(definition));document.querySelector('#save-state').textContent='Saved on this device';toast('Draft saved on this device.');}catch(error){toast(`Draft could not be saved. ${error.message}`);}
};
...
document.querySelector('#import-file').onchange=async e=>{
  ...
  try{
    ...
    definition=readDefinition(JSON.parse(await file.text()));changed();setView('builder');toast('Definition opened.');
  }catch(error){toast(`Could not open definition. ${error.message}`);}finally{e.target.value='';}
};
try {
  const responses=await Promise.all([fetch('./catalog.json'),fetch('./cohort_engine.sas')]);
  if(responses.some(r=>!r.ok))throw new Error('Unable to load the schema or SAS engine.');
  catalog=await responses[0].json();engine=await responses[1].text();
  let stored;
  try{stored=localStorage.getItem(DRAFT_KEY);}catch(error){toast(`Local draft storage is unavailable. ${error.message}`);}
  if(stored){try{definition=readDefinition(JSON.parse(stored));document.querySelector('#save-state').textContent='Saved on this device';}catch(error){definition=freshDefinition();toast(`The saved draft could not be loaded. ${error.message}`);}}
  render();
