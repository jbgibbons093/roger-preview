import { DOMAINS as CDM_DOMAINS, ENC_TYPES } from './cdm.js?v=dce5f8e2fe76';
import { treeFor, groupsIn, logicText, moveCondition } from './logic.js?v=dce5f8e2fe76';
...
export function issueNode(issue){
  ...
  if(/\boutcome\b|post-index outcomes/i.test(text))return 'outcomes';
  if(/\bcovariate\b|baseline covariates|predictor/i.test(text))return 'covariates';
...
function graphModel(d,catalog){
  ...
  add('outcomes','Post-index outcomes','stage',`${d.outcomes?.length||0} outcome${d.outcomes?.length===1?'':'s'}\nFirst event and date after index\nFollow-up status retained`,270,finalRow);
  add('output','Selected population','stage',`One row per person\n${d.outputs.length?d.outputs.join(', ')+' extracts':'Cohort and audit tables'}\nCounts available after SAS execution`,510,finalRow);
  ...
  edges.push([tree.id,'covariates'],['covariates','outcomes'],['outcomes','output']);
...
export function bindTree(container,d,catalog,{changed,refresh,addRule,removeRule,browseCodes,notify,domains,issues}){
  ...
  const outcomeFields=()=>`<div class="tree-form"><p class="hint">Each outcome uses the first matching event from day 1 or later. A no-event flag requires complete medical enrollment through the horizon with zero gap.</p>${(d.outcomes||[]).map((out,index)=>`<details class="tree-covariate" open><summary>${esc(out.label||out.key)}</summary><div class="tree-field-grid">${field('Variable key','outcome','key',out.key,'text','maxlength="20"',index)}${field('Display label','outcome','label',out.label,'text','maxlength="80"',index)}</div>${selectField('Code system','outcome','domain',out.domain,Object.keys(CDM_DOMAINS).map(key=>[key,catalog.domains[key].label]),index)}<div class="tree-field"><label for="tree-outcome-${index}-codes">Codes</label><textarea id="tree-outcome-${index}-codes" data-tree-model="outcome" data-tree-index="${index}" data-tree-key="codes" rows="2">${esc(out.codes)}</textarea><button type="button" class="button small" data-tree-outcome-browse="${index}">Browse code catalog</button></div><div class="tree-field-grid">${field('From day','outcome','from',out.from,'number','min="1" step="1"',index)}${field('Through day','outcome','to',out.to,'number','min="1" step="1"',index)}</div>${out.domain==='NDC'?'':`<div class="tree-field"><span class="field-label">Encounter types</span><div class="tree-checks">${Object.entries(ENC_TYPES).map(([value,label])=>`<label><input type="checkbox" data-tree-outcome-enc="${index}" value="${value}" ${out.encTypes.includes(value)?'checked':''}>${value} · ${esc(label)}</label>`).join('')}</div></div>`}<button type="button" class="button small danger" data-tree-outcome-remove="${index}">Remove outcome</button></details>`).join('')||'<p class="hint">No outcomes selected.</p>'}<button type="button" class="button full-button" data-tree="add-outcome" ${(d.outcomes||[]).length>=20?'disabled':''}>+ Add outcome</button></div>`;
  function inspector(){
    ...
    const scopeContent=selected==='covariates'?`<div class="tree-form"><h4>Predictors</h4><label class="tree-toggle"><input type="checkbox" data-tree-predictor-built-in="age" ${d.predictors?.age!==false?'checked':''}>Age at index</label><label class="tree-toggle"><input type="checkbox" data-tree-predictor-built-in="sex" ${d.predictors?.sex!==false?'checked':''}>Recorded sex</label>${d.covariates.map((cov,index)=>`<label class="tree-toggle"><input type="checkbox" data-tree-predictor-cov="${index}" ${cov.predictor===true?'checked':''}>${esc(cov.label||cov.key)} · cov_${esc(cov.key)}</label>`).join('')}${d.comparison?`<h4>Covariate groups</h4>${d.covariates.map((cov,index)=>selectField(cov.label||cov.key,'cov','arm',cov.arm||'BOTH',[['BOTH','Shared'],['TREATMENT','Treatment'],['CONTROL','Control']],index)).join('')}`:''}</div>`:'';
  ...
  function editValue(el){
    ...
    const target=kind==='rule'?(index===-1?d.index:d.rules[index]):kind==='control'?d.comparison.controlIndex:kind==='comparison'?d.comparison:kind==='cov'?d.covariates[index]:kind==='outcome'?d.outcomes[index]:kind==='map'?d.mapping:d;
  ...
  container.addEventListener('click',e=>{
    ...
    const outcomeBrowse=e.target.closest('[data-tree-outcome-browse]');if(outcomeBrowse){browseCodes('outcome',Number(outcomeBrowse.dataset.treeOutcomeBrowse));return;}
    ...
    const outcomeRemove=e.target.closest('[data-tree-outcome-remove]');if(outcomeRemove){d.outcomes.splice(Number(outcomeRemove.dataset.treeOutcomeRemove),1);changed();redraw();return;}
    ...
    if(action==='add-covariate'){
      ...
      d.covariates.push({key:`feature_${n}`,label:`Feature ${n}`,domain:'DX',sources:['DIA'],codes:'',encTypes:Object.keys(ENC_TYPES),from:-365,to:-1,minDays:1,predictor:true});
      changed();redraw();return;
    }
    if(action==='add-outcome'){
      d.outcomes||=[];if(d.outcomes.length>=20){notify('A cohort can contain up to 20 outcomes.');return;}
      const n=d.outcomes.length+1;d.outcomes.push({key:`outcome_${n}`,label:`Outcome ${n}`,domain:'DX',sources:['DIA'],codes:'',encTypes:Object.keys(ENC_TYPES),from:1,to:30});changed();redraw();return;
    }
  ...
  container.addEventListener('change',e=>{
    ...
    if(el.dataset.treePredictorBuiltIn!==undefined){d.predictors||={age:true,sex:true};d.predictors[el.dataset.treePredictorBuiltIn]=el.checked;changed();reflect();return;}
    if(el.dataset.treePredictorCov!==undefined){d.covariates[Number(el.dataset.treePredictorCov)].predictor=el.checked;changed();reflect();return;}
    if(el.dataset.treeOutcomeEnc!==undefined){const out=d.outcomes[Number(el.dataset.treeOutcomeEnc)];out.encTypes=el.checked?[...out.encTypes,el.value]:out.encTypes.filter(value=>value!==el.value);changed();reflect();return;}
    ...
    if(el.dataset.treeModel){
      ...
      if(el.dataset.treeModel==='outcome'&&el.dataset.treeKey==='domain'){
        const out=d.outcomes[Number(el.dataset.treeIndex)];out.sources=CDM_DOMAINS[out.domain].slice();out.codes='';out.encTypes=out.domain==='NDC'?[]:Object.keys(ENC_TYPES);
      }
