import { isCdm, DOMAINS as CDM_DOMAINS, ENC_TYPES } from './cdm.js?v=13d0f862af21';
import { treeFor, groupsIn, logicText, moveCondition } from './logic.js?v=13d0f862af21';
...
export function bindTree(container,d,catalog,{changed,refresh,addRule,removeRule,browseCodes,notify,domains,issues}){
  ...
  function inspector(){
    ...
    const content=selected==='population'?`<div class="tree-form">${field('Cohort name','cohort','name',d.name)}${isCdm(d)?`<div class="tree-field-grid">${field('Delivery start year','cohort','yearStart',d.yearStart,'number','min="1900" max="2100"')}${field('End year','cohort','yearEnd',d.yearEnd,'number','min="1900" max="2100"')}</div>`:`<div class="tree-field-grid">${selectField('Database','cohort','family',d.family,Object.entries(catalog.families))}${selectField('Delivery set','cohort','edition',d.edition,[['A','Set A'],['B','Set B']])}</div>`}</div>`:selected==='index'?ruleFields(d.index,-1):isRule?ruleFields(d.rules[Number(selected.slice(1))],Number(selected.slice(1))):selected==='demographics'?`<div class="tree-form"><div class="tree-field-grid">${field('Minimum age','cohort','ageMin',d.ageMin,'number','min="0" step="1"')}${field('Maximum age','cohort','ageMax',d.ageMax,'number','min="0" step="1"')}</div>${selectField('Recorded sex','cohort','sex',d.sex,isCdm(d)?[['ALL','Any'],['M','Male'],['F','Female'],['A','Ambiguous'],['U','Unknown']]:[['ALL','Any'],['1','Male'],['2','Female']])}</div>`:selected==='enrollment'?`<div class="tree-form"><label class="tree-toggle"><input type="checkbox" data-tree-model="cohort" data-tree-key="enrollment" ${d.enrollment?'checked':''}>Require ${isCdm(d)?'medical coverage':'continuous enrollment'}</label><div class="tree-field-grid three">${field('Days before','cohort','baseline',d.baseline,'number','min="0" step="1"')}${field('Days after','cohort','followup',d.followup,'number','min="0" step="1"')}${field('Maximum gap','cohort','gap',d.gap,'number','min="0" step="1"')}</div><label class="tree-toggle"><input type="checkbox" data-tree-model="cohort" data-tree-key="rx" ${d.rx?'checked':''} ${!d.enrollment?'disabled':''}>Require ${isCdm(d)?'drug coverage':'pharmacy capture'}</label></div>`:selected==='covariates'?covariateFields():selected==='output'?outputFields():'';
  ...
  container.addEventListener('change',e=>{
    ...
    if(el.dataset.treeModel){
      if(el.dataset.treeModel==='rule'&&el.dataset.treeKey==='domain'){
        ...
        rule.sources=domains[rule.domain].slice(0,isCdm(d)?1:2);rule.codes='';if(isCdm(d))rule.encTypes=rule.domain==='NDC'?[]:Object.keys(ENC_TYPES);
      }
