...
import { isCdm, requiredTables, expandMapping } from './cdm.js';
...
function cdmProtocol(d,catalog){
  ...
  return [
    ...
    errors.length?`DRAFT. Resolve before execution\n${errors.map(e=>'- '+e).join('\n')}`:'Definition validated for program generation. SAS execution and population counts are pending.',
    'SOURCE AND DELIVERY',
    `Institutional Mini-Sentinel CDM files for delivery years ${d.yearStart} through ${d.yearEnd}. Table structure follows the supplied Mini-Sentinel_Common-Data-Model.pdf v3.0. Filename patterns follow Jun’s delivery note. These configured years require verification against the actual delivery. Required annual tables are scanned across every configured year using the record date. File-year labels describe provenance and impose no separate calendar-year filter on records.`,
    'INDEX AND SELECTION ORDER',
    `Select one ${order} matching index date per character PatID. Same-day ties use source filename, EncounterID, and normalized code in ascending order. Apply demographic, enrollment, and condition-tree filters after choosing index. Failure removes the person without substituting another candidate date. Patient identifiers retain leading zeros.`,
    event(d.index,0),
    'DEMOGRAPHIC REQUIREMENTS',
    `Join Demographic by PatID. The demographic source must have at most one row per nonmissing PatID. Repeated IDs stop the run. Age is completed years from recorded Birth_Date through index_date, calculated with SAS INTCK('year',Birth_Date,index_date,'c'). Age must be ${d.ageMin} through ${d.ageMax}, inclusive. Missing demographic records, missing birth dates, and birth dates after index are excluded. Recorded Sex ${d.sex==='ALL'?'has no additional restriction':`must equal '${d.sex}'`}.`,
    'OBSERVATION AND ENROLLMENT',
    ...
    'The Enrollment_abd rollup is assumed to expose the standard CDM interval fields. SAS preflight checks required field names and character/numeric types. Actual content, date units, completeness, and CDM conformance require institutional validation. Death dates never shorten enrollment in this definition.',
    'CONDITION TREE',logicText(tree),
