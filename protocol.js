import { treeFor, logicText, usesOr } from './logic.js?v=20580f35e743';
import { validateDefinition, parseCodes } from './cohort.js?v=20580f35e743';
import { requiredTables, expandMapping } from './cdm.js?v=20580f35e743';
...
export function selectionProtocol(d,catalog){
  ...
  const event=(r,i,arm='')=>{
    ...
    const start=arm==='CONTROL'?d.comparison.start:d.start,end=arm==='CONTROL'?d.comparison.end:d.end;
    return `${i?`Criterion ${i}`:arm?`${arm==='CONTROL'?'Control':'Treatment'} index event`:'Index event'}\n${catalog.domains[r.domain].label}. ${catalog.domains[r.domain].matching}. Source ${r.sources.map(t=>catalog.tables[t].label).join(', ')}.\n${r.domain==='NDC'?'Date is RxDate.':`Date is ADate. Accepted EncType values are ${r.encTypes.join(', ')}.`}\n${i?`${r.mode==='INCLUDE'?'Require':'Exclude people with'} at least ${r.minDays} distinct event day(s) from index day ${r.from} through ${r.to}, inclusive.`:`Select the ${order} matching event between ${start} and ${end}, inclusive.`}\nCodes, combined with OR: ${codes||'[codes required]'}`;
  };
  let files;
  try{
  ...
  return [
    ...
    d.comparison?`Select the ${order} qualifying event separately for treatment and control. Keep both candidate dates (treatment_index_date and control_index_date) and assign one selected index_date and index_arm per PatID. ${d.comparison.overlap==='EXCLUDE'?'Exclude anyone with both candidate arms.':'If both arms qualify, use the earlier arm-specific candidate date; a same-day tie goes to treatment.'} Demographic, enrollment, and condition-tree filters follow the arm assignment; failure does not substitute the other arm or another event. SAS retains the source PatID type and one final row per person.`:`Select one ${order} matching index date per PatID. SAS retains the identifier type from the source and requires the same type across selected tables. Same-day ties use source filename, EncounterID, and normalized code in ascending order. Apply demographic, enrollment, and condition-tree filters after choosing index. Failure removes the person without substituting another candidate date. Character identifiers retain leading zeros when present.`,
    event(d.index,0,d.comparison?'TREATMENT':''),
    ...(d.comparison?[event(d.comparison.controlIndex,0,'CONTROL')]:[]),
